...
export function FriendsRequestsClient() {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h3 className="text-[22px] leading-[1.27] tracking-[-0.25px] text-[rgba(0,0,0,0.95)] dark:text-[rgba(255,255,255,0.95)]">{dict.friendsSection.requestsTitle}</h3>
        <p className="text-sm text-[#615d59] dark:text-[#bbb6af]">{dict.friendsSection.requestsDescription}</p>
      </div>

      {error ? <p className="text-sm text-[#dd5b00] dark:text-[#ff8f63]">{error}</p> : null}
      {loading && !payload ? <p className="text-sm text-[#615d59] dark:text-[#bbb6af]">{dict.friendsSection.loadingRequests}</p> : null}

      <div className="space-y-2">
        <h4 className="text-sm font-semibold text-[rgba(0,0,0,0.95)] dark:text-[rgba(255,255,255,0.95)]">{dict.friendsSection.incoming}</h4>
        {!loading && payload && payload.incoming.length === 0 ? (
          <p className="text-sm text-[#615d59] dark:text-[#bbb6af]">{dict.friendsSection.noIncoming}</p>
        ) : null}
          ...
          >
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0">
                <p className="truncate text-sm font-semibold text-[rgba(0,0,0,0.95)] dark:text-[rgba(255,255,255,0.95)]">{request.name}</p>
                <p className="text-xs text-[#615d59] dark:text-[#bbb6af]">{dict.friendsSection.requestedAt.replace("{{value}}", formatRequestTime(request.createdAt))}</p>
              </div>
            </div>
      ...
      <div className="space-y-2">
        <h4 className="text-sm font-semibold text-[rgba(0,0,0,0.95)] dark:text-[rgba(255,255,255,0.95)]">{dict.friendsSection.outgoing}</h4>
        {!loading && payload && payload.outgoing.length === 0 ? (
          <p className="text-sm text-[#615d59] dark:text-[#bbb6af]">{dict.friendsSection.noOutgoing}</p>
        ) : null}
        {payload?.outgoing.map((request) => (
          ...
          >
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0">
                <p className="truncate text-sm font-semibold text-[rgba(0,0,0,0.95)] dark:text-[rgba(255,255,255,0.95)]">{request.name}</p>
                <p className="text-xs text-[#615d59] dark:text-[#bbb6af]">{dict.friendsSection.sentAt.replace("{{value}}", formatRequestTime(request.createdAt))}</p>
              </div>
            </div>
            <span className="rounded-full bg-[#f6f5f4] px-2 py-1 text-xs font-semibold text-[#615d59] dark:bg-[#23211f] dark:text-[#bbb6af]">{dict.friendsSection.pendingBadge}</span>
          </div>
        ))}
      </div>
